...
class RnnRecommender(nn.Module):
    def __init__(self, embed_dim, num_layers, num_items, pad_value =0):
        ...
        # Embedding layer
        self.embeds = nn.Embedding(num_items, self.embed_dim, padding_idx=pad_value)

        # LSTM layers
        self.lstm = nn.LSTM(batch_first = True, input_size=self.embed_dim, hidden_size=self.embed_dim, num_layers = self.num_layers, dropout=0.1)

        # Linear layer
        self.item2item = nn.Linear(self.embed_dim, num_items)
    
    def forward(self, inputs):
        embeds = self.embeds(inputs)
        lstm_out, _ = self.lstm(embeds.view(len(inputs), 1, -1))
        item_space = self.item2item(lstm_out.view(len(inputs), -1))
...
if __name__ == "__main__":
    ...
    # Train model
    model = RnnRecommender(embed_dim, num_layers, len(unique_m))
